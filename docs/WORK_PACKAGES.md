...
`docs/OPERATIONS.md` (ADR-033): גיבוי + תרגיל שחזור רבעוני · monitoring · Dependabot · incident response · DPAs + מיקום מידע · מדיניות שמירה 7ש'/2ש' מול anonymize+lock · "תיק חירום" להמשכיות · **תיקון 13 לחוק הגנת הפרטיות** (רישום בוטל; חובת ניהול/אבטחה — רובה מיושמת; DPO כנראה לא נדרש — אישור עו"ד) · checklist פרודקשן.
...
מפתח ב-env · `crypto.subtle`/libsodium · מיגרציה להצפנת רשומות קיימות.
**DoD:** `totp_secret` מוצפן במנוחה; enroll/verify עובדים; בדיקה.
**תלוי:** WP-02, WP-21

### WP-25 · Retention cron ⬜
`/api/cron/purge` מוגן `CRON_SECRET` → `purgeOldAudit` + גיזום `login_attempt`/הזמנות פגות · `crons` ב-`vercel.ts`.
**DoD:** הרצה מתוזמנת יומית; רשומה מעל 730 יום נגזמת; append-only trigger שורד.
**תלוי:** WP-05, WP-21

### WP-26 · Anonymize + lock (מחיקת מטופל) ⬜
מסך/פעולה: `status=inactive` + חסימת `user` + אנונימיזציית שדות מזהים אחרי 7ש' · נרשם ב-audit.
**DoD:** מטופל "נמחק" לא נגיש; תוכן קליני נשמר עד תום השמירה; בדיקת בידוד.
**תלוי:** WP-10, WP-21

### WP-27 · `pg_dump` שבועי אוטומטי ⬜
GitHub Action / Vercel Cron → dump ל-Blob/יעד מוצפן · שמירת 4 עותקים חודשיים.
