...
**תוספת — בלוקים סגורים ביומן המטפלת:** `/t/calendar` (agenda שבועי) מציג עכשיו גם את חסימות ה-busy
מ-Google לצד הפגישות הפנימיות, ממוינות יחד לפי שעה. פריט Google מוצג כשורה מוחשכת עם אייקון מנעול
("תפוס ביומן Google"), ללא קישור (אי אפשר לפתוח פרטים — אין לנו אותם). **דה-דופ:** פגישה של Nofar
שנדחפה ל-Google תיראה גם ב-free/busy של Google עצמו; מזוהה לפי התאמת `[start,end]` מדויקת מול הפגישות
הפנימיות של אותו שבוע ומדולגת, כדי שלא תופיע כפול. אומת בדפדפן מול נתונים אמיתיים מיומן ה-Google
המחובר של הלקוחה — דה-דופ עבד נכון (אפס כפילויות על 3 הפגישות הפנימיות שהיו באותו שבוע).
