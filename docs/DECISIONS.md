...
## ADR-053 — הוורוד (blush) = מצב "דורש תשומת לב / פעולה"
**תאריך:** 2026-09-03 · **סטטוס:** נעול

הלקוח שאל למה הוורוד (#e9d4d5) כמעט לא בשימוש. ניסינו לרגע כלל "דו-גוני
בדאשבורד" (מרווה = התקדמות, ורוד = אישי) — הלקוח בחן וחזר: *"הורוד באמת מתאים
למקומות שבהם חסר מידע / התראות / משהו שהלקוח או המטופל צריך לעשות."* זה גם
עקבי עם ADR-012 (`blush` נדיר ומכוון, אקסנט לא-סמנטי).

**כלל:** `blush-soft` מסמן **פריט פתוח שדורש פעולה מהמשתמש**. שאר המדדים
נשארים `sage-tint` רגוע. דחיפות אמיתית (איחור) עדיין `text-danger` הסמנטי.

- **רשת KPI של המטופל** (`patient-kpis.tsx`): כל 4 האריחים `sage-tint`
  כברירת מחדל. "משימות פתוחות" → `blush-soft` **רק** כשיש משימה באיחור או
  להיום. מספר האיחור עצמו → `text-danger`.
- **שורת ה"חכם"** → `bg-blush-soft` (היא *עצם* ה"תעשה עכשיו"). הצ'יפ נשאר
  `sage-deep` (כפתור פעולה בצבע המותג).
- **באנר שאלון ממתין** (`p/page.tsx`) → `blush-soft` (היה `sage-soft`).
- **דשבורד המטפלת** (`t/page.tsx`): מצב `highlight` (התראות שלא נקראו) →
  `blush-soft` + צ'יפ אייקון `bg-blush` (היה `sage-soft`). שאר האריחים מרווה.

הבאדג'ים הקיימים (פעמון, מוני ניווט) כבר `bg-blush` — נשארים. לא הוגדרו
tokens חדשים. אומת ב-375px (מטופל) וב-1280px (מטפלת).
