...
### 2026-09-03 (המשך ב') — הוורוד = "דורש פעולה" (ADR-053)
הלקוח שאל למה הוורוד (#e9d4d5) לא בשימוש. אחרי ניסיון קצר של "דו-גוני בדאשבורד" הוחלט: `blush-soft` מסמן **פריט פתוח שדורש פעולה מהמשתמש**, שאר המדדים נשארים מרווה רגוע. באנר שאלון ממתין → ורוד · שורת ה"חכם" → ורוד · אריח "משימות פתוחות" → ורוד רק כשיש איחור/להיום (מספר האיחור `text-danger`) · דשבורד מטפלת: `highlight` (התראות שלא נקראו) → ורוד. באדג'ים קיימים (פעמון/מונים) כבר ורוד. typecheck + lint + 174 בדיקות + build ✓. אומת ב-375px וב-1280px.

### 2026-09-03 (המשך) — דאשבורד KPI חכם במסך הבית של המטופל (ADR-052)
מוקאפ הוצג ואושר. `patient-kpis.tsx` — רשת `grid-cols-2 md:grid-cols-4`, כל אריח `<Link>`: טיפולים בסדרה (used/total + פס + נותרו) · משימות פתוחות (רקע בלאש כשדחוף, אדום לאיחור) · הפגישה הבאה ("היום/מחר/עוד N ימים" + יום·שעה·תחום) · יומן אכילה (X/7 השבוע + 7 נקודות + עודכן היום?). **שורת "חכם"** בוחרת פריט דחוף אחד לפי עדיפות (איחור → סדרה ≤2 → להיום → יומן לא עודכן היום → כלום) עם צ'יפ פעולה. הרשת החליפה את 3 כרטיסי הסטטוס הנפרדים; באנר שאלון + "עדכונים אחרונים" נשארו. typecheck + lint + 174 בדיקות + build ✓. אומת ב-375px וב-1280px.

### 2026-09-03 — הגדלת טיפוגרפיה ופקדים במובייל (ADR-051)
הלקוח: "במובייל הכל קטן מדי — הכתב, האייקונים, הצ'קבוקסים." תיקון תחום ל-`< 768px` בלבד, דסקטופ ללא שינוי (`md:` מחזיר כל ערך). `globals.css`: root 16→17.5px במובייל + פקדי טופס נעולים ל-16px (בלי zoom ב-iOS). Checkbox 16→20px + שטח נגיעה ≥44px. גבהי כפתורים: default 32→40 · sm 28→36 · lg 36→44 · icon 32→36. Input 32→40. סרגל תחתון + אייקוני ניווט 21→24. פעמון+יציאה בסרגל העליון 32→36 (זכוכית 18→20). אייקוני שורה 13–15→16, תוויות `text-[11px]`→`text-xs` בכל מסכי המטופל. typecheck + lint + 174 בדיקות + build ✓. אומת בדפדפן ב-375px (צ'קבוקס 22px נמדד, כפתור שליחה 44px) וב-1280px (דסקטופ ללא שינוי). קודם: מלל דף הכניסה עודכן ל"נופר כהן נטורופתית N.D והרבליסטית קלינית Cl.H".
